...
* 如果是https协议，还要进行TLS四次握手：
    ...
    2. 服务器接受到请求后，返回 选中的加密方法、安全证书 和 随机数 server_rand，server_param 给 浏览器

    3. 浏览器接收到响应后，查验安全证书是否有效（信任的CA？域名一致？未过期？），如果有效，生成 client_param，结合第二步的加密算法和 server_param 生成 secret，同时将 client_param 以及结合前边随机数生成的加密内容给服务器以验证服务器的身份。
    4. 服务器 利用加密算法 、client_param、server_param 生成secret。
