...
* 同步，程序在调用一个函数或方法后，需要一直等待返回值或者消息的接受后才会往下继续执行其他命令
* 异步，程序调用函数或方法后，不必阻塞等待，只需向系统委托一个异步过程，在接受到返回值或消息后，系统自动触发委托的过程。

[怎么理解阻塞非阻塞同步非同步](https://www.zhihu.com/question/19732473)

[同步异步、阻塞非阻塞](https://segmentfault.com/a/1190000016827291)



### 28. 浏览器的缓存机制

* 浏览器的缓存机制指的是，浏览器在一段时间内保留接收到的web资源的副本，如果在有效时间内，对资源再一次请求，那么浏览器就会使用缓存的副本而不必向服务器请求。
* 使用缓存可以有效提高打开页面的速度，同时减少服务器网络带宽的消耗，降低服务器压力。
* 
